...
import com.benlawrencem.game.beleagame.Beleagame;
...
public class EntityPerspective implements Perspective {
	...
	public void render(Graphics g, Entity entity) {
		float x = Beleagame.GAME_WIDTH / 2 + (entity.getX() - this.entity.getX()) * level.getTileWidth() * scale;
		float y = Beleagame.GAME_HEIGHT / 2 + (entity.getY() - this.entity.getY()) * level.getTileHeight() * scale;
		Visibility visibility = Visibility.VISIBLE;
		if(x < -MAX_OBJECT_WIDTH * scale || x > Beleagame.GAME_WIDTH + MAX_OBJECT_WIDTH * scale)
			visibility = Visibility.HIDDEN;
		if(y < -MAX_OBJECT_HEIGHT * scale || y > Beleagame.GAME_HEIGHT + MAX_OBJECT_HEIGHT * scale)
			visibility = Visibility.HIDDEN;
	...
	public void render(Graphics g, Tile tile) {
		float x = Beleagame.GAME_WIDTH / 2 + (tile.getX() - entity.getX()) * level.getTileWidth() * scale;
		float y = Beleagame.GAME_HEIGHT / 2 + (tile.getY() - entity.getY()) * level.getTileHeight() * scale;
		Visibility visibility = Visibility.VISIBLE;
		if(x < -MAX_OBJECT_WIDTH * scale || x > Beleagame.GAME_WIDTH + MAX_OBJECT_WIDTH * scale)
			visibility = Visibility.HIDDEN;
		if(y < -MAX_OBJECT_HEIGHT * scale || y > Beleagame.GAME_HEIGHT + MAX_OBJECT_HEIGHT * scale)
